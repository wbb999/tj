				...
				<table border="0" align="center" cellpadding="0" cellspacing="0"
					...
					<tr height="20px;">
						<td style="color: green">${errowMesg}<c:if test="${flag==0}">订单状态更新失败
										</c:if> <c:if test="${flag==1}">订单状态更新成功，已邮件通知用户！
										</c:if></td>
					</tr>
					<tr>
						<td>
									...
									<tbody>
										...
										<s:iterator value="#request.ordertables" id="exa" status="num">
											<tr>
												...
												<td><c:if test="${exa.state.id==1}">
														<a
															href="ordertable_receorder?&oneorder.id=<s:property value="#exa.id"/>">接单</a>
													</c:if> <c:if test="${exa.state.id!=1}">接单</c:if> | <c:if
														test="${exa.state.id==1}">
														<a
															href="ordertable_unorder?&oneorder.id=<s:property value="#exa.id"/>">取消订单</a>
													</c:if> <c:if test="${exa.state.id!=1}">取消订单</c:if> | <a
													href="ordertable_single?&oneorder.id=<s:property value="#exa.id"/>">详情</a>
													| <c:if test="${exa.state.id!=1&&exa.state.id!=2}">
														<a
															href="ordertable_delorder?&oneorder.id=<s:property value="#exa.id"/>"
															onclick="return del()">删除</a>
													</c:if> <c:if test="${exa.state.id==1||exa.state.id==2}">删除</c:if>
													| <c:if test="${exa.state.id==2}">
														<a
															href="ordertable_downorder?&oneorder.id=<s:property value="#exa.id"/>">结束订单</a>
													</c:if> <c:if test="${exa.state.id!=2}">结束订单</c:if>
											</tr>
										</s:iterator>
									</tbody>
								</table>
								...
								<c:if test="${ordertables.size()!=0}">
									<p
										...
										<s:if test="#page.hasFirst">
											<a href="ordertable_keysearch?pageNow=1&key=${key}&searchtype=${searchtype}">首页</a>
										</s:if>
										<s:if test="#page.hasPre">
											<a
												href="ordertable_keysearch?pageNow=<s:property value="#page.pageNow-1"/>&key=${key}&searchtype=${searchtype}">上一页</a>
										</s:if>
										<s:if test="#page.hasNext">
											<a
												href="ordertable_keysearch?pageNow=<s:property value="#page.pageNow+1"/>&key=${key}&searchtype=${searchtype}">下一页</a>
										</s:if>
										<s:if test="#page.hasLast">
											<a
												href="ordertable_keysearch?pageNow=<s:property value="#page.totalPage"/>&key=${key}&searchtype=${searchtype}">尾页</a>
										</s:if>
									</p>
								</c:if>
							</div>
