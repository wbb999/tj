...
<script type="text/javascript">
	function del(id) {
		if (confirm("确定删除常见问题吗？")) {
			location.href = "nomalquestion_delete?&question.id=" + id;
		} else {
			return false;
		}
	}
								...
								<c:if test="${questions.size()!=0}">
									<p
										...
										<s:if test="#page.hasFirst">
											<a href="nomalquestion_allactByPage?pageNow=1&key=${key}">首页</a>
										</s:if>
										<s:if test="#page.hasPre">
											<a
												href="nomalquestion_allactByPage?pageNow=<s:property value="#page.pageNow-1"/>&key=${key}">上一页</a>
										</s:if>
										<s:if test="#page.hasNext">
											<a
												href="nomalquestion_allactByPage?pageNow=<s:property value="#page.pageNow+1"/>&key=${key}">下一页</a>
										</s:if>
										<s:if test="#page.hasLast">
											<a
												href="nomalquestion_allactByPage?pageNow=<s:property value="#page.totalPage"/>&key=${key}">尾页</a>
										</s:if>
									</p>
								</c:if>
							</div>
