							...
							<table width="90%" border="0px" align="center" cellpadding="0"
								...
								<tbody>
									<c:if test="${questions.size()==0}">
										<tr>
											<td style="color: red;">对不起！未查询到与该关键字相关的问题!
												&nbsp;&nbsp;&nbsp;&nbsp; <input type="button" value="返回"
												onclick="history.go(-1)"
												style="border: 0; background-color: #fff; color: blue" />
											</td>
										</tr>
									</c:if>
								...
								<c:if test="${questions.size()!=0}">
									<p
										...
										<s:if test="#page.hasFirst">
											<a href="search_search?searchtype=2&pageNow=1&key=${key}">首页</a>
										</s:if>
										<s:if test="#page.hasPre">
											<a
												href="search_search?searchtype=2&pageNow=<s:property value="#page.pageNow-1"/>&key=${key}">上一页</a>
										</s:if>
										<s:if test="#page.hasNext">
											<a
												href="search_search?searchtype=2&pageNow=<s:property value="#page.pageNow+1"/>&key=${key}">下一页</a>
										</s:if>
										<s:if test="#page.hasLast">
											<a
												href="search_search?searchtype=2&pageNow=<s:property value="#page.totalPage"/>&key=${key}">尾页</a>
										</s:if>
									</p>
								</c:if>
							</div>
