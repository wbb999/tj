								...
								<c:if test="${service_products.size()!=0}">
									<p
										...
										<s:if test="#page.hasFirst">
											<a
												href="service_product_keysearch?pageNow=1&key=${key}">首页</a>
										</s:if>
										<s:if test="#page.hasPre">
											<a
												href="service_product_keysearch?pageNow=<s:property value="#page.pageNow-1"/>&key=${key}">上一页</a>
										</s:if>
										<s:if test="#page.hasNext">
											<a
												href="service_product_keysearch?pageNow=<s:property value="#page.pageNow+1"/>&key=${key}">下一页</a>
										</s:if>
										<s:if test="#page.hasLast">
											<a
												href="service_product_keysearch?pageNow=<s:property value="#page.totalPage"/>&key=${key}">尾页</a>
										</s:if>
									</p>
								</c:if>
							</div>
